...
describe('job artifacts api route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('passes viewer-derived defaults into the platform context before auth checks', async () => {
    const { GET } = await import('@/app/api/jobs/[id]/artifacts/route');

    const response = await GET(new NextRequest('https://app.test/api/jobs/job_1/artifacts'), {
  ...
  it('allows access with a valid generation access token even without a viewer', async () => {
    ...
    mocks.getPlatformRuntime.mockReturnValue({
      ...
      generationArtifacts: {
        listByJobId: vi.fn().mockResolvedValue([{ id: 'artifact_1' }]),
      },
    });
